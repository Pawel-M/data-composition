...
class WeightingMarianConfig(MarianConfig):
    model_type = "marian"
    ...
    def __init__(
            ...
            weighting_amount=0.0,
            weighting_token_level=False,
            **kwargs,
    ):
        self.weighting_amount = weighting_amount
        self.weighting_token_level = weighting_token_level
...
class WeightingMarianMTModel(MarianMTModel):
    def __init__(self, config: WeightingMarianConfig):
    ...
    def forward(
        ...
        masked_lm_loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss(reduction='none', ignore_index=self.config.pad_token_id)
            masked_lm_loss = loss_fct(lm_logits.view(-1, self.config.decoder_vocab_size), labels.view(-1))
            masked_lm_loss = masked_lm_loss.view(labels.shape)
            if exp_mask is not None:
                if self.config.weighting_token_level:
                    loss_mask = exp_mask
                else:
                    loss_mask = torch.sum(exp_mask, dim=-1, keepdim=True).broadcast_to(exp_mask.shape)
                masked_lm_loss = masked_lm_loss * (loss_mask * self.config.weighting_amount + 1)
