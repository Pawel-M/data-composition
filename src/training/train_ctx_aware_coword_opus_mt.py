...
from modeling.coword_opus_mt import CoWordMarianConfig, CoWordMarianMTModel
...
def tokenize_all_with_context_and_sentence(tokenizer,
    ...
    for src, tgt, src_tok, tgt_tok, src_ctxs, tgt_ctxs in zip(sources, targets, all_tokenized['input_ids'],
        ...
        for src_ctx in src_ctxs + [src]:
            ...
            if num_additional_src_tokens > 0 and index == len(src_ctxs):
                src_ctx_len += 1
        ...
        for tgt_ctx in tgt_ctxs + [tgt]:
            ...
            if num_additional_src_tokens > 0 and index == len(tgt_ctxs):
                tgt_ctx_len += 1
...
if __name__ == '__main__':
    ...
    tokenizer.add_special_tokens({'sep_token': '</sep>', 'mask_token': '<mask>'})

    model_config_kwargs = config.get('model_config_arguments', {})
    model_config = CoWordMarianConfig.from_pretrained(
        model_name,
    ...
    if random_initialization:
        ...
        model = CoWordMarianMTModel(model_config)
    else:
        model = CoWordMarianMTModel.from_pretrained(model_name, config=model_config)
