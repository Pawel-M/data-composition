...
def score(translations, eval_set, pretty=False, complete=False):
    """
    Scores a set of translations against a set of rules.
    """

    references = get_test_data(eval_set)
    # translations = get_translation_stream(translations)
    ...
    for translation, ref in zip(translations, references):
        ...
        last_sentence = translation.strip()
    ...
    if pretty:
        ...
        for rule, scores in score_rules.items():
            print(
                f"{rule} ({scores['form']})\t{scores['correct']}\t{scores['total']}\t{scores['correct'] * 100 / scores['total']:.1f}%")
        print("-------------------------------------------")
